...
export class BookingService {
  ...
  setDates(start: string | Date, end?: string | Date) {
    const startStr = typeof start === 'string' ? start : start.toISOString().slice(0, 10);
    const endStr = end ? (typeof end === 'string' ? end : end.toISOString().slice(0, 10)) : undefined;
    this.selection.date = startStr;
    if (endStr && endStr !== startStr) {
      this.selection.endDate = endStr;
    } else {
